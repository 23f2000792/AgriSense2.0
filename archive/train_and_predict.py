...
def train_and_predict():
    ...
    visit_log = pd.read_csv(os.path.join(DATA_DIR, 'retailer_visit_log.csv'))
    
    print("Building True Features & Ranking Formula...")
    # 1. Retailer Value Potential
    ...
    ret_df = retailers.merge(pos_agg, on='retailer_id', how='left')\
                      .merge(inv_agg, on='retailer_id', how='left').fillna(0)
    
    ret_df['velocity'] = ret_df['total_qty'] / 30.0  # Daily velocity
    ret_df['days_of_cover'] = ret_df['current_stock'] / (ret_df['velocity'] + 1)
    
    # 2. Score Calculation: Score = w1*Value + w2*Risk + w3*Relationship + w4*Opportunity + w5*Urgency
    scaler = StandardScaler()
    ret_df['Value_Potential'] = scaler.fit_transform(ret_df[['total_value']])
    ret_df['Risk'] = scaler.fit_transform((1 / (ret_df['days_of_cover'] + 1)).values.reshape(-1, 1)) # higher risk = lower days of cover
    
    # Coverage Urgency: Days since last visit (mocked based on territory distribution)
    np.random.seed(42)
    ...
    ret_df['Final_Score'] = (w1 * ret_df['Value_Potential'] + 
                             w2 * ret_df['Risk'] + 
                             w3 * ret_df['Relationship'] + 
                             w4 * ret_df['Opportunity'] + 
                             w5 * ret_df['Coverage_Urgency'])
    
    ret_df = ret_df.sort_values('Final_Score', ascending=False)
    ret_df['priority'] = pd.qcut(ret_df['Final_Score'], 3, labels=['Low', 'Medium', 'High'])
    
    print("Training XGBoost Classifier for Next Best Action (NBA)...")
    # 3. XGBoost Next-Best-Action Model
    ...
    ret_df['pred_nba_class'] = xgb_model.predict(X_nba)
    sku_mapping = dict(enumerate(pd.factorize(ret_df['sku_id'])[1]))
    ret_df['pred_nba_sku'] = ret_df['pred_nba_class'].map(sku_mapping)
    ...
    print("Running Isolation Forest for Anomaly Alerts...")
    ...
    print("Exporting Production JSON to Frontend...")
    output_visits = []
    for _, row in ret_df.head(20).iterrows():
        ...
        nba = {
            'product': product,
            'objective': 'Urgent Replenishment' if is_risk else 'Cross-sell / Volume Expansion',
            'roi': f"₹{int(max(1000, row['total_value'] * 0.15)):,} Expected",
            'talkingPoints': ['Stock will deplete in <10 days', f'Competitor supply for {product} is low'] if is_risk else [f'High recent transaction volume for {product}', 'Offer bulk discount'],
        ...
        output_visits.append({
            ...
            'priority': row['priority'], 'reason': reason, 'tehsil': row['tehsil'],
            'sales30d': f"₹{int(row['total_value']):,}", 'daysCover': int(row['days_of_cover']),
            'score': round(row['Final_Score'], 2),
            'nba': nba
        })

    # Basic mapping for growers from WA Campaign
    ...
    for _, row in grw_df.head(10).iterrows():
        ...
        try:
            if isinstance(row['grower_crop_calendar'], str):
                ...
                if stages:
                    stage = stages[0].get('stage', 'Vegetative').capitalize()
        except: pass
            
        nba = {
            'product': 'Amistar Top' if 'flowering' in stage.lower() else 'Karate Zeon',
            'objective': 'Disease protection' if 'flowering' in stage.lower() else 'Pest control & Growth',
            'roi': f"Protect {int(row['farm_size_num'] * 15)}% yield",
            'talkingPoints': [f'Critical for {crop} during {stage} stage', 'High engagement on last WhatsApp campaign'],
        ...
        output_visits.append({
            ...
            'reason': f"High Potential - {stage}", 'tehsil': row['tehsil'],
            'crop': crop, 'nba': nba
        })
    ...
    print(f"✅ Training and inference complete. Predictions saved to {OUT_FILE}")
