            ...
            <div style={{
              ...
              <div style={{ padding: '0 1.25rem 1.25rem 1.25rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  <button className="btn" style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8' }}
                    onClick={async (e) => {
                      e.stopPropagation();
                      // Optimistic remove
                      onVisitLogged && onVisitLogged(visit.id, "Skipped — outlet closed or rep unavailable");
                      try {
                        await fetch(`/api/visits/${visit.id}/skip`, {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ reason: "Skipped by rep via app" })
                        });
                      } catch(e) {}
                    }}
                  >
